...
def get_pair_corr(input_name, r_max, bin_spacing, multi, option1, option2):
        ...
        if option1 == 'In':
            s = np.where(atom == 'In')[0]
        elif option1 == 'Ga':
            s = np.where(atom == 'Ga')[0]
        elif option1 == 'Zn':
        ...
        g = open('pcf_'+str(option1)+'.out', 'w')

        for i in range(len(r)):
            templine = str(r[i]) + '\t' + str(pair_corr[i]) + '\n'
            g.write(templine)
...
def average_Nc_bc(input_name, r_cutoff, multi, option1, option2):
        ...
        for i in range(s_1+1, s_2 + 1):
            ...
            if temp == 0:
                ...
                final_set[7].append(i)
            else:
                print 'CRITICAL ERROR: coordination number of ' + str(i) + ' is out of range!!!'
                print 'Its coordination number is ' + str(temp)
        
        #print final_set
        len_final_set = [len(final_set[0]),len(final_set[1]),len(final_set[2]),len(final_set[3]),len(final_set[4]),len(final_set[5]),len(final_set[6]),len(final_set[7])]
...
def plot_LIPR(final_list, plot_energy_range,criteria, VT_position, VBM_position, CBM_position, option=0):
    ...
    for i in range(nkpt):
        for j in range(nband):
            templine = str(final_list[i][1][j]-corr) + '\t' + str(final_list[i][2][j]) +'\n'
            g.write(templine)
    g.close()
...
Ef, VBM_position, total_out_dos, projected_out_dos = DOS('CONTCAR', 'DOSCAR', np.array([1]), option=1)
...
#r, pair_corr = get_pair_corr('CONTCAR', 5, 0.1, np.array([2, 2, 2]), 'In', 'O')
#plot_pair_corr(r, pair_corr)
#r, pair_corr = get_pair_corr('CONTCAR', 5, 0.1, np.array([2, 2, 2]), 'Ga', 'O')
#r, pair_corr = get_pair_corr('CONTCAR', 5, 0.1, np.array([2, 2, 2]), 'Zn', 'O')

average_Nc_bc('CONTCAR', 2.5, np.array([2, 2, 2]), 'In', 'O')
average_Nc_bc('CONTCAR', 2.3, np.array([2, 2, 2]), 'Ga', 'O')
